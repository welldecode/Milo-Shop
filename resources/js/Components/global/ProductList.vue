...
<template>
    <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-3 lg:grid-cols-6 xl:gap-x-8">
        ...
        <div v-for="product in products" :key="product.id" class="group relative">
            <!-- add to cart icon -->
            <div class="absolute right-5 top-10 flex items-center  gap-3 opacity-0 group-hover:opacity-100 cursor-pointer transition-all duration-300 ">
            ...
            <div class="mt-4 flex justify-between gap-5">
                <div class="flex flex-col  ">
                    ...
                    <span class="text-base font-bold text-gray-900">R$ {{ product.price }}</span>
                </div>
            </div>
            </Link>
        </div>
